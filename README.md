...
| `pnpm typecheck`    | `next typegen && tsc --noEmit`. `next typegen` genera los tipos globales del App Router (p. ej. `LayoutProps`) en `.next/types/` sin correr un build completo, así que el script funciona standalone en un checkout limpio. CI sigue usando `pnpm build` como type-check real (cubre también errores que solo aparecen durante el build). |
