...
| Errores parciales de parseo de precios         | `reportUploadSuccess` (`app.js` L726-743): preview de hasta 5 direcciones | `UploadDropzone` mostraba solo el conteo agregado — regresión de paridad real (KAN-302: el agente no podía ver qué corregir en el Excel), corregida listando hasta 5 filas (`hoja: dirección ("valor crudo")`) + "y N más", igual que el legacy | `upload-dropzone.test.tsx` (KAN-302: detalle por fila + caso >5 errores)                                                              | ✅     |
